...
const ChatBox = ({ toggleChatbot }) => {
    ...
    const sendMessage = async (message = input) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { text: "Unable to fetch response.", sender: "bot" }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                exit={{ opacity: 0, y: 50 }}
                className="fixed bottom-0 right-0 md:bottom-32  z-50 
                w-full md:w-96 h-full md:h-[550px] bg-white rounded-t-xl md:rounded-xl shadow-xl flex flex-col"
            >
                {/* HEADER */}
