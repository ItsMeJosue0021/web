...
const Knowledgebase = () => {
    ...
    const header = {
        title: "Knowledgebase Management",
        subTitle: "Effortlessly manage your knowledgebase — add, update, or remove entries."
    };
    ...
    return (
        <Admin header={header} breadcrumbs={breadcrumbs}>
            ...
            <div className="w-full flex flex-col gap-4 mt-4">
                ...
                {/* TOP BAR */}
                <div className="bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-lg border border-gray-100 gap-3">
                    
                    {/* Search */}
                    <div className="w-full sm:max-w-[500px] flex items-center gap-3">
                        <p className="text-xs">Search</p>
                        <input 
                            onChange={(e) => handleSearch(e.target.value)} 
                            type="text" 
                            className="w-full bg-white placeholder:text-xs px-4 py-2 rounded border border-gray-200 text-sm" 
                            placeholder="Search for Knowledgebases.." 
                        />
                    </div>
                    ...
                    <div>
                        {!isOpen ? (
                            ...
                            <button 
                                className="bg-gray-200 hover:bg-gray-300 text-xs px-4 py-2 rounded w-full sm:w-auto"
                                onClick={() => {
                                    setIsOpen(false);
                                    setSelectedId(null);
                                    setFormData({ title: "", content: "" });
                ...
                {isOpen && (
                    <form 
                        ...
                        className="flex flex-col gap-3 p-4 bg-white rounded-lg border border-gray-100"
                    >
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                name="title"
                                placeholder="Your knowledgebase title here.."
                                className="w-full bg-white text-xs px-4 py-2 rounded-md border border-gray-300"
                                value={formData.title}
                                onChange={handleChange}
                            />
                            {errors.title && <p className="text-red-500 text-xs">{errors.title[0]}</p>}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <textarea
                                name="content"
                                placeholder="Your knowledgebase content here.."
                                className="w-full bg-white text-xs h-28 px-4 py-2 rounded-md border border-gray-300"
                                value={formData.content}
                                onChange={handleChange}
                            />
                            {errors.content && <p className="text-red-500 text-xs">{errors.content[0]}</p>}
                        ...
                        <button 
                            type="submit" 
                            className="w-fit text-xs px-4 py-2 rounded bg-orange-600 text-white"
                        >
                            {isSaving ? "Saving..." : selectedId ? "Update" : "Save"}
                        </button>
                    </form>
                ...
                <div className="w-full overflow-x-auto rounded-lg border border-gray-100">
                    <table className="bg-white text-xs w-full border-collapse">
                        ...
                        {!loading && (
                            <tbody>
                                {knowledgebase.map((row, index) => (
                                    <tr key={row.id} className={`${index % 2 === 0 ? "bg-orange-50" : ""}`}>
                                        <td className="p-3">{row.title}</td>
                                        <td className="p-3 text-xs">{row.content}</td>
                                        <td className="p-3 flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className="bg-red-50 text-red-600 px-2 py-1 rounded"
                                            >
                                                <Trash2 size={16} />
                                            </button>
                                        </td>
                    ...
                    <AnimatePresence>
                        ...
                        >
                            <div className="bg-white p-6 rounded-md shadow-lg w-full max-w-sm">
                                <h3 className="text-base font-semibold mb-2 text-red-500">Confirm Deletion</h3>
                                <p className="text-xs text-gray-600">Are you sure you want to delete this entry?</p>
